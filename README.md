...
- **Film Room** — load a match video from your disk and tag timestamped moments (goals, sprints, tackles, mistakes…) while you watch, then jump back to any moment. The video never leaves your device — only your tags are saved, and they feed the AI coach's match analysis. Benchmark sessions chart per-test progression (40yd, vertical, broad jump…) on the dashboard.
- **End-to-end-encrypted device sync** — export a single `.ppsync` file encrypted with your passphrase (AES-256-GCM, PBKDF2 key derivation) *before* it leaves the app, move it over any channel you already own (iCloud/Dropbox/USB/e-mail), and import it on another device. Merge is additive and dedup'd — nothing gets overwritten, and there's no PitchPace server in the middle.
